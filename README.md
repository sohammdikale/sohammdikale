<div align="center">

# 👋 Hi, I'm **Soham Dikale**

### `AI & Data Science Student` • `AI/ML Enthusiast` • `Python Developer`

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=22&pause=1000&color=58A6FF&center=true&vCenter=true&width=600&lines=Artificial+Intelligence+%26+Data+Science;Machine+Learning+%7C+Deep+Learning;Generative+AI+%7C+RAG+%7C+Computer+Vision;Python+%7C+SQL+%7C+Java;Always+Learning+%26+Building+%F0%9F%9A%80" alt="Typing SVG" />

<br>

<a href="https://github.com/sohammdikale">
  <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" />
</a>
<a href="https://www.linkedin.com/in/soham-dikale-910592342/">
  <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" />
</a>

</div>

---

## 🧑‍💻 About Me

* 🎓 B.Tech student specializing in **Artificial Intelligence & Data Science**
* 🤖 Passionate about **AI, Machine Learning, Deep Learning & Generative AI**
* 🐍 Strong interest in **Python development and AI engineering**
* 📊 Exploring **Data Science, SQL and data-driven solutions**
* 👁️ Interested in **Computer Vision, NLP and RAG systems**
* 🚀 Love building practical projects and experimenting with new technologies
* 📚 Constantly learning, improving and exploring the world of AI

---

## ⚡ Tech Stack

### 👨‍💻 Languages

<p align="left">
<img src="https://skillicons.dev/icons?i=python,c,java,javascript,html,css" />
</p>

### 🤖 AI / ML

<p align="left">
<img src="https://skillicons.dev/icons?i=tensorflow,pytorch,opencv" />
</p>

**Machine Learning • Deep Learning • NLP • Computer Vision • Generative AI • RAG • YOLO**

### 🗄️ Database & Backend

<p align="left">
<img src="https://skillicons.dev/icons?i=mysql,firebase,supabase,flask" />
</p>

### 🛠️ Tools & Platforms

<p align="left">
<img src="https://skillicons.dev/icons?i=git,github,vscode,streamlit" />
</p>

---

## 📈 GitHub Stats

<div align="center">

<img height="170" src="https://github-readme-stats.vercel.app/api?username=sohammdikale&show_icons=true&hide_border=true&rank_icon=github&theme=transparent" />

<img height="170" src="https://github-readme-stats.vercel.app/api/top-langs/?username=sohammdikale&layout=compact&hide_border=true&theme=transparent" />

</div>

---

## 🔥 Contribution Streak

<div align="center">

<img src="https://streak-stats.demolab.com?user=sohammdikale&hide_border=true&theme=transparent" />

</div>

---

## 🐍 Contribution Activity

<div align="center">

<img src="https://raw.githubusercontent.com/Platane/snk/output/github-contribution-grid-snake.svg" alt="GitHub Contribution Snake" />

</div>

---

## 🎯 2026 Goals

```text
☑ Strengthen Machine Learning fundamentals
☑ Build production-ready AI applications
☑ Explore Generative AI & RAG
☑ Improve Data Structures & Algorithms
☑ Contribute to Open Source
☑ Grow as an AI Engineer
```

---

<div align="center">

### 💡 *"Build. Learn. Experiment. Repeat."*

<br>

<img src="https://komarev.com/ghpvc/?username=sohammdikale&style=for-the-badge&color=58A6FF&label=PROFILE+VIEWS" />

<br><br>

**Thanks for visiting my profile! 🚀**

</div>
